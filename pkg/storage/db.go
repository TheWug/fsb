...
func ClearAliasIndex(d DBLike) (error) {
	return d.Enter(func(tx Queryable) error { return WrapExec(tx.Exec("TRUNCATE alias_index")) })
}
...
func AliasUpdater(d DBLike, input chan apitypes.TAliasData) (error) {
	...
	return d.Enter(func(tx Queryable) error {
		for alias := range input {
			...
			if err := WrapExec(tx.Exec(sql, alias.Id)); err != nil { return err }

			sql = "INSERT INTO alias_index (alias_id, alias_name, alias_target_id) SELECT $1, $2, tag_id FROM tag_index WHERE tag_name = $3"
			if err := WrapExec(tx.Exec(sql, alias.Id, alias.Name, alias.Alias)); err != nil { return err }
		}
...
func SetCatsException(d DBLike, tag string) (error) {
	...
	return d.Enter(func(tx Queryable) error { return WrapExec(tx.Exec(sql, tag)) })
}
...
func RecalculateAliasedCounts(d DBLike) (error) {
	query := "UPDATE tag_index " +
			"SET tag_count = subquery.tag_count, tag_count_full = subquery.tag_count_full" +
		"FROM (SELECT a.tag_id, c.tag_count, c.tag_count_full " +
	...
	return d.Enter(func(tx Queryable) error { return WrapExec(tx.Exec(query)) })
}
...
func GetBlits(d DBLike, yes, no, wild bool) ([]BlitData, []BlitData, []BlitData, error) {
	...
	if err != nil {
		out_yes, out_no, out_wild = nil, nil, nil
	}
	return out_yes, out_no, out_wild, err
}
...
func FindPromptPost(d DBLike, id int) (*PromptPostInfo, error) {
	...
	err := d.Enter(func(tx Queryable) error {
		return tx.QueryRow(query, id).Scan(&x.PostId, &x.PostType, &x.PostURL, &x.SampleURL, &x.PostMd5, &x.PostWidth, &x.PostHeight, &x.MsgId, &x.ChatId, &x.Timestamp, &x.Captioned, &x.Edit)
	})
...
func FindPromptPostByMessage(d DBLike, chat_id tgtypes.ChatID, msg_id tgtypes.MsgID) (*PromptPostInfo, error) {
	...
	err := d.Enter(func(tx Queryable) error {
		return tx.QueryRow(query, chat_id, msg_id).Scan(&x.PostId, &x.PostType, &x.PostURL, &x.SampleURL, &x.PostMd5, &x.PostWidth, &x.PostHeight, &x.MsgId, &x.ChatId, &x.Timestamp, &x.Captioned, &x.Edit)
	})
...
func SavePromptPost(d DBLike, id int, x *PromptPostInfo) (error) {
	...
	return d.Enter(func(tx Queryable) error {
		// XXX use upsert
		if err := WrapExec(tx.Exec(query, id)); err != nil { return err }
		...
		if x != nil {
			...
			return WrapExec(tx.Exec(query, id, x.PostType, x.PostURL, x.SampleURL, strings.ToLower(x.PostMd5), x.PostWidth, x.PostHeight, x.MsgId, x.ChatId, x.Timestamp, x.Captioned, x.Edit))
		}
